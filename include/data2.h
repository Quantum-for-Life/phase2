 ...
 *
 * After a successful call, the value pointed to by the argument "num_steps"
 * stores the number of steps in the "time_series" group.  If the
 * values cannot be read, the function returns '-1', and the value of the
 * variables poited to remains unchanged.
 *
 * Arguments:
 *
 *  fid			Open file id obtained from data2_open()
 *  num_steps	Pointer to a variable where the result will be stored.
 *
 * Return value:
 *
 *   0			if the value was successfully retrieved
 ...
 * and a complex number representing value of the time series; and a generic
 * pointer to the data specified by the user.
 *
 * The return value of the operator "op" controls the iteration.  If "op"
 * returns "0", the iteration will continue with the next element.  If the
 * return value is non-zero, the iteration will stop and the value is returned
