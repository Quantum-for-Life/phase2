...
class Case:
    ...
    def generate_pauli_hamil(self, num_terms: int):
        ...
        matrix = pauli_hamil_to_matrix(coeffs, paulis)
    ...
    def generate_time_series(self, num_steps: int):
        ...
        for i in range(0, num_steps):
            self.time_series["times"][i] = int(random.random() * num_steps)
    ...
    def write_info(self):
        case_info = {
            ...
            "pauli_hamil": {
                ...
                "eigs": list(self.pauli_hamil["eigs"]),
                # "eivecs": [
                #     [v[i] for i in range(0, len(v))] for v in self.pauli_hamil["eivecs"]
                # ],
            },
            "multidet": {
            ...
            "time_series": {
                ...
                "values": [[z.real, z.imag] for z in self.time_series["values_comp"]],
            },
        }
        with open(self.filename_prefix + ".json", "w") as f:
            json_str = json5.dumps(case_info, indent=4, allow_nan=True)
            # print(json_str)
            f.write(json_str)
...
if __name__ == "__main__":
    ...
    if not args.many:
        ...
        case.generate_time_series(args.num_steps)
        case.compute_values()
        case.write_h5file()
        case.write_info()
    else:
        for i in range(0, args.many):
            case = Case(FILENAME_STUB, random.randrange(2, args.num_qubits))
            case.generate_pauli_hamil(
                random.randrange(1, min(args.num_terms, 2**case.num_qubits))
            )
            case.generate_multidet(
                random.randrange(1, min(args.num_dets, 2**case.num_qubits))
            )
            case.generate_time_series(random.randrange(1, args.num_steps))
            case.write_h5file()
            case.write_info()
