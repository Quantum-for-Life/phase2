...
void capi_hadamard(struct circ *c, qbid qb)
{
	Qureg qureg = circ_intl_quest_qureg(c);
	hadamard(qureg, qb);
}

void capi_sgate(struct circ *c, qbid qb)
{
	Qureg qureg = circ_intl_quest_qureg(c);
	sGate(qureg, qb);
}

double capi_prob0(struct circ *c, qbid qb)
{
	Qureg qureg = circ_intl_quest_qureg(c);
	return calcProbOfOutcome(qureg, qb, 0);
}

void capi_blankstate(struct circ *c)
{
	Qureg qureg = circ_intl_quest_qureg(c);
	initBlankState(qureg);
}

void capi_set_sysamp(struct circ *c, size_t idx, _Complex double amp)
{
	Qureg qureg = circ_intl_quest_qureg(c);
...
void capi_ctl_rotate_pauli(struct circ *c, int *paulis, double angle)
{
	Qureg qureg = circ_intl_quest_qureg(c);
	...
	multiControlledMultiRotatePauli(qureg, qb_ctl, num_mea_qb, qb_trg,
					(enum pauliOpType *)paulis, num_sys_qb,
					-2.0 * angle);
}