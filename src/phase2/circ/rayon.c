...
int rayon_routine(struct circ *c) {
        struct rayon_circuit_data *ct_dat = (struct rayon_circuit_data *)
                c->ct.data;
        struct rayon_circ_data *dat = (struct rayon_circ_data *) c->data;
        ...
        for (size_t r = 0; r < (size_t) REPS; r++) {
                for (int i = 0; i < ct_dat->hamil.numSumTerms; i++) {
                        ...
                        qreal angle = 2.0 / time * ct_dat->hamil.termCoeffs[i];
...
int rayon_state_post(struct circ *c) {
        ...
        struct rayon_circ_data *d = (struct rayon_circ_data *) c->data;
        if (d->imag_switch == 1) {
