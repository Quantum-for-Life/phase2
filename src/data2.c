...
/* --- State prep --- */
static int
state_prep_open(const data2_id fid, hid_t *grpid)
{
	const hid_t hid = H5Gopen2(fid, DATA2_STATE_PREP, H5P_DEFAULT);
	if (hid == H5I_INVALID_HID)
		return -DATA2_ESTPREP;
	*grpid = hid;
...
static int
multidet_open(const data2_id fid, struct multidet_handle *md)
{
	hid_t sp_id;
	...
	if (state_prep_open(fid, &sp_id) < 0)
		return -DATA2_ESTPREP;
	const hid_t md_id =
		H5Gopen2(sp_id, DATA2_STATE_PREP_MULTIDET, H5P_DEFAULT);
	if (md_id == H5I_INVALID_HID) {
		state_prep_close(sp_id);
		return -DATA2_EMULDET;
	}
...
int
data2_hamil_foreach(const data2_id fid,
	...
	data2_id fid, double *values[2], size_t num_values)
{
	...
	const hid_t dset =
		H5Dopen2(grpid, DATA2_TROTTER_STEPS_VALUES, H5P_DEFAULT);
